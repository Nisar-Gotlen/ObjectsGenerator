...
def change_bright(ob, mat, brightName, brCont):

    copyMat = mat.copy()
    copyMat.use_nodes = True
    copyMat.node_tree.nodes.active
    copyMat.node_tree.nodes[brightName].inputs["Bright"].default_value = brCont
    return copyMat


def gen_radish(ob, matrixSize, matrixShift, bend_div, taper_div, scale_factor):
    pose_x = 0
    for y in range(0, matrixSize):
        ...
        for x in range(0, matrixSize):
            ...
            change_noise(copy, copy.data.materials[0], NoiseName, noise_scale)
            for i in range(len(copy.data.materials)):
                ob.data.materials[i] = change_bright(copy,
                                                     copy.data.materials[i],
                                                     brightName, brCont)

            x = 1 + random.uniform(-copy.dimensions.x * scale_factor,
                                   copy.dimensions.x * scale_factor)
            #y = 1 + random.uniform(-copy.dimensions.y*scale_factor, copy.dimensions.y*scale_factor)
            z = random.uniform(copy.dimensions.z - 0.2,
                               copy.dimensions.z + 0.3)
            copy.dimensions.z = z
            #copy.scale = (x, x, z)
            pose_y += matrixShift
...
gen_radish(obTuber, matrixSize, matrixShift, 5, 4, 0.02)
'''
